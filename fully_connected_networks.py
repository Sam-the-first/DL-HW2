...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dz = dout
        dw = (x_reshape.t()@dz)
        db = torch.sum(dz,0,keepdim=False)
        dx = dz@w.t()
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        x_reshape = x.reshape(x.shape[0],-1)
        z = x_reshape@w + b
        out = torch.max(torch.zeros_like(z),z)
        cache = (x, w, b, z)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
...
def softmax_loss(x, y):
    ...
    ######################################################################
    # Replace "pass" statement with your code
    sum_ = torch.sum(torch.exp(x),1)
    y_hat = torch.div(torch.exp(x),sum_.reshape(-1,1))
    loss = torch.tensor([0],device=y_hat.device,dtype=y_hat.dtype)
    i = 0
    dx = y_hat.clone()
    for yi,y_hati in (zip(y,y_hat)):
        loss+=torch.log(y_hati[yi])
        dx[i,yi]-=1
        i+=1
    # print(loss.item())
    loss = -loss/x.shape[0]
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        Y1 ,cache1 = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        Y2 ,cache2 = Linear.forward(Y1, self.params['W2'], self.params['b2'])
        scores = Y2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        d1, dw2, db2 = Linear.backward(dout, cache2)
        dw2 += 2*self.reg*self.params['W2']
        dx, dw1, db1 = Linear_ReLU.backward(d1, cache1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if i == self.num_layers-1:
                ...
                if self.use_dropout:
                    scores, cache_drop = Dropout.forward(scores, self.dropout_param)
        ...
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers-1:
                ...
                dw += 2*self.reg*self.params[f'W{i+1}']
                grads[f'W{i+1}'] = dw
                grads[f'b{i+1}'] = db
            else:
                if self.use_dropout:
                    cache, cache_drop = caches[i]
                    dout = Dropout.backward(dout, cache_drop)
                else:
                    cache = caches[i]
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape,dtype=x.dtype,device=x.device) <(1-p))/(1-p)
            out = mask * x
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
